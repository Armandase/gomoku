...
bool Game::operator==(Game& rhs) {
  if (getClassicBoard() == rhs.getClassicBoard()) return true;
  return false;
}
...
ClassicBoard& Game::getClassicBoard() noexcept { return (this->_classicBoard); }
...
DiagBoard& Game::getDiagBoard() noexcept { return ((this->_diagBoard)); }
...
size_t Game::hashGame() const {
  std::size_t h1 =
      std::hash<IBoard::bitboard>{}(this->_classicBoard.getPlayer1());
  std::size_t h2 =
      std::hash<IBoard::bitboard>{}(this->_classicBoard.getPlayer2());
  return h1 ^ (h2 << 1);
}

void Game::setHeuristic(int64_t heuristic) { this->_heuristic = heuristic; }

int64_t Game::getHeuristic() const { return this->_heuristic; }

bool Game::isFull() const {
  IBoard::bitboard fullBoard =
      this->_classicBoard.getPlayer1() | this->_classicBoard.getPlayer2();
  return (fullBoard.count() ==
          this->_classicBoard.getWidth() * this->_classicBoard.getWidth());
}

bool Game::canBeCaptured(uint16_t x, uint16_t y, Game::PatternType boardType,
                         uint16_t player) {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (!getClassicBoard().isValidPos(x, y)) continue;
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if (getClassicBoard().isValidPos(xOpp1, yOpp1) &&
          getClassicBoard().isValidPos(xFwd2, yFwd2)) {
        if ((getClassicBoard().getPos(xOpp1, yOpp1) == opponent &&
             getClassicBoard().getPos(xFwd1, yFwd1) == player &&
             getClassicBoard().getPos(xFwd2, yFwd2) == EMPTY) ||
            (getClassicBoard().getPos(xOpp1, yOpp1) == EMPTY &&
             getClassicBoard().getPos(xFwd1, yFwd1) == player &&
             getClassicBoard().getPos(xFwd2, yFwd2) == opponent)) {
          return true;
        }
      }
      ...
      if (getClassicBoard().isValidPos(xOpp2, yOpp2) &&
          getClassicBoard().isValidPos(xRev2, yRev2)) {
        if ((getClassicBoard().getPos(xOpp2, yOpp2) == opponent &&
             getClassicBoard().getPos(xRev1, yRev1) == player &&
             getClassicBoard().getPos(xRev2, yRev2) == EMPTY) ||
            (getClassicBoard().getPos(xOpp2, yOpp2) == EMPTY &&
             getClassicBoard().getPos(xRev1, yRev1) == player &&
             getClassicBoard().getPos(xRev2, yRev2) == opponent)) {
          return true;
        }
      }
    }
...
bool Game::playerWin(uint16_t player) {
  if (getCapture(player) >= 5) return true;
...
Game::patternMap Game::extractPatterns(uint16_t x, uint16_t y, uint16_t length,
                                       uint16_t player) {
  patternMap result;

  result.insert({Game::CLASSIC,
                 this->_classicBoard.extractPattern(x, y, length, player)});
  result.insert({Game::TRANSPOS,
  ...
  result.insert({Game::REV_CLASSIC, this->_classicBoard.extractPatternReversed(
                                        x, y, length, player)});
  result.insert(
      {Game::REV_TRANSPOS,
       this->_transposedBoard.extractPatternReversed(x, y, length, player)});
  result.insert({Game::REV_DIAG, this->_diagBoard.extractPatternReversed(
                                     x, y, length, player)});
  result.insert(
      {Game::REV_ANTIDIAG,
...
bool Game::isDoubleThree(uint16_t x, uint16_t y, uint16_t player) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (opponentPatternMap[boardType] == 0 &&
        (playerPatternMap[boardType] == playerPattern1 ||
         playerPatternMap[boardType] == playerPattern2 ||
         playerPatternMap[boardType] == playerPattern3))
      doubleThreeCnt++;
    else if ((opponentPatternMap[boardType] == 1 ||
              opponentPatternMap[boardType] == 16) &&
             playerPatternMap[boardType] == playerPattern1)
      doubleThreeCnt++;
    else if (opponentPatternMap[boardTypeRev] == 0 &&
             (playerPatternMap[boardTypeRev] == playerPattern1 ||
              playerPatternMap[boardTypeRev] == playerPattern2 ||
              playerPatternMap[boardTypeRev] == playerPattern3))
      doubleThreeCnt++;
    else if ((opponentPatternMap[boardTypeRev] == 1 ||
              opponentPatternMap[boardTypeRev] == 16) &&
             playerPatternMap[boardTypeRev] == playerPattern1)
      doubleThreeCnt++;
  }
...
std::vector<uint16_t> Game::isCapture(uint16_t x, uint16_t y, uint16_t player) {
  ...
  for (int i = 0; i < 8; i++) {
    Game::PatternType boardType = static_cast<Game::PatternType>(i);
    if (playerPatternMap[boardType] == playerPattern &&
        opponentPatternMap[boardType] == opponentPattern)
...
bool Game::canCapture(uint16_t x, uint16_t y, uint16_t player) {
  if (!getClassicBoard().isPosEmpty(x, y)) return false;
...
void Game::handleCapture(uint16_t x, uint16_t y,
                         std::vector<uint16_t>& capturesBoard, uint16_t player,
                         Render& render) {
  const int dirX[8] = {1, 0, 1, 1, -1, 0, -1, -1};
  const int dirY[8] = {0, 1, -1, 1, 0, -1, 1, -1};
...
bool checkPatternAtPosition(const patternMerge& playerLine,
                            const patternMerge& opponentLine, t_pattern pattern,
                            int startPos) {
  if (startPos < 0 || startPos > MERGE_SIZE) return false;
  patternMerge mask((1 << pattern.length) - 1);

  patternMerge playerShiftedPattern(playerLine.to_ulong() >> (9 - startPos));
  patternMerge opponentShiftedPattern(opponentLine.to_ulong() >>
                                      (9 - startPos));
  ...
  return playerShiftedPattern == pattern.player &&
         opponentShiftedPattern == pattern.opponent;
}
...
int Game::heuristicTest(int x, int y, int player) {
  ...
  for (const t_pattern& pattern : patternsArray) {
    for (int i = 0; i < 4; i++) {
      patternBitset boardPlayerPattern =
          extractPlayer[static_cast<Game::PatternType>(i)];
      patternBitset revBoardPlayerPattern =
          extractPlayer[static_cast<Game::PatternType>(i + 4)];
      patternMerge mergedPlayerPattern = (boardPlayerPattern.to_ulong() << 4 |
                                          revBoardPlayerPattern.to_ulong());

      patternBitset boardOpponentPattern =
          extractOpponent[static_cast<Game::PatternType>(i)];
      patternBitset revBoardOpponentPattern =
          extractOpponent[static_cast<Game::PatternType>(i + 4)];
      patternMerge mergedOpponentPattern =
          (boardOpponentPattern.to_ulong() << 4 |
           revBoardOpponentPattern.to_ulong());

      for (int i = 0; i < pattern.length; i++) {
        if (checkPatternAtPosition(mergedPlayerPattern, mergedOpponentPattern,
                                   pattern, 5 - i)) {
