...
int getCurrentPlayer(int depth, int initPlayer) {
  if ((depth + 2) % 2 == DEPTH % 2) return initPlayer;
...
IBoard::bitboard getSurroundingBits(Game& game) {
  ...
  for (int y = 0; y < board_size; y++) {
    for (int x = 0; x < board_size; x++) {
      ...
      if (allPieces.test(x + y * board_size) == true) continue;

      for (int i = 0; i < 8; i++) {
        int checkX = x + dir[i][0];
        int checkY = y + dir[i][1];
        if (checkX < 0 || checkY < 0 || checkX >= board_size ||
            checkY >= board_size)
          continue;

        surroundingBits |=
            (IBoard::bitboard(1) << (checkX + checkY * board_size));
      }
